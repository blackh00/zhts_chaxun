	...
	for (i in sFtv)
		if (sFtv[i].match(/^(\d{2})(\d{2})([\s\*|\s\#|\s@])(.+)\s+(.+)$/))
			if (Number(RegExp.$1) == (m + 1)) {
				...
				if(y<1949&&m==9&&Number(RegExp.$2)==1){
					this[Number(RegExp.$2) - 1].solarFestival = '';
					this[Number(RegExp.$2) - 1].color='';
				}
			}
...
function calendarForOne(y, m, d) {
	...
	if(y==2013&&m==1&&d==3){
		...
		holiday[5]='立春|jieqi.htm?y=2013&jq='+escape('立春');
	}else{
		holiday[5]=getStermDay(y,m+1,d)+'|jieqi.htm?y='+y+'&jq='+escape(getStermDay(y,m+1,d));
	}
