...
function getcityid($city){
	$file = fopen(ROOT_PATH."/App/Home/data/Cityid.id","r");   //打开文件CItyid.id
	while (!feof($file))			//循环遍历文件
	{
		$bruce=fgets($file);			//逐行读入文件内容
		if(strpos($bruce,$city)) 		//字符判断
		{  
			$id = substr($bruce,0,9);			//输出本行前9个字符
			break;
		}
	}
	fclose($file);			//关闭文件
...
function object_to_array($obj){  
      ...
      foreach ($_arr as $key => $val){  
             $val = (is_array($val) || is_object($val)) ? $val=object_to_array($val) : $val;  
             $arr[$key] = $val;  
      }  
      return $arr;  
} 
...
获取汉字笔画
**/
 function getbihua($obj){  
	$bihua = D('bihua');
	$where ="hanzi like '%".$obj."%'";
	$field = 'bihua';
	$data = $bihua->where($where)->field($field)->select(); 
	$bh = $data['0']['bihua'];
	return $bh;  
} 
/**
 ...
 * @return string $html
 */
function getSafeCode($value, $code = 'utf-8') {//$code=gb2312
	if (is_array($value)) {
...
function checkStr($str){
    ...
    $b=ereg('[0-9]', $str);
    $c=ereg('[a-zA-Z]', $str);
    if($a && $b && $c){ $output        = 1;}  //汉字数字英文的混合字符串
    elseif($a && $b && !$c){ $output   = 2;}  //汉字数字的混合字符串
    elseif($a && !$b && $c){ $output   = 3;}  //汉字英文的混合字符串
    elseif(!$a && $b && $c){ $output   = 4;}  //数字英文的混合字符串
    elseif($a && !$b && !$c){ $output  = 5;}  //纯汉字
    elseif(!$a && $b && !$c){ $output  = 6;}  //纯数字
    elseif(!$a && !$b && $c){ $output  = 7;}  //纯英文
    return $output;
}
/**
 * PHP过滤所有空格，回车，换行
