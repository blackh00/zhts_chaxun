...
function setTplAvatarFormat($id,$type)
{
	...
	if($id == 0)
	{
		...
		{
			case 'm':
				return './Www/Uploads/avatar/noavatar_middle.jpg';
			break;
			case 'b':
				return './Www/Uploads/avatar/noavatar_big.jpg';
			break;
			default:
				return './Www/Uploads/avatar/noavatar_small.jpg';
			break;
		}
	}
...
function getCacheIsUpdate($cache_file,$time_out,$is_clear = 0)
{
	...
	if($is_clear == 1)
	{
		$clear_path = ROOT_PATH.'/Data/is_clear.lock';
...
{
	$dir = './Www/Uploads/temp/'.fToDate(NULL,'Y/m/d/H');
	if(empty($key))
...
function avatar($img_id, $type = 'm',$is_full = false)
{
	...
	if($img_id == 0)
	{
		...
		{
			case 'm':
				$src = './Www/Uploads/avatar/noavatar_middle.jpg';
			break;
			case 'b':
				$src = './Www/Uploads/avatar/noavatar_big.jpg';
			break;
			default:
				$src = './Www/Uploads/avatar/noavatar_small.jpg';
			break;
		}
...
function copyFile($origin_path, $path = 'share',$del_temp = true,$id = 0)
{
	...
	{
		if($id > 0)
			$dir = './Www/Uploads/'.$path.'/'.getDirsById($id);
		else
			$dir = './Www/Uploads/'.$path.'/'.fToDate(NULL,'Y/m/d');
	}
...
function cssParse($urls)
{
	...
	if(is_array($urls))
	{
		...
		if(!file_exists($url_path))
		{
			...
			foreach($urls as $url)
			{
				$url = str_replace('./tpl/css/','./Data/tpl/css/',$url);
