...
class LotteryAction extends Action{
	...
	public function shuangseqiuApi(){
		...
		for($i=$qishi;$i<$num;$i++){
			$prize = array();
			$redballs = array();
			$dangqianqinum = $_GET['qishu']-$i;
			if(strlen($dangqianqinum)<5){
				$dangqianqinum = "0".$dangqianqinum;
			}
			
			$url = "http://kaijiang.500.com/shtml/ssq/".$dangqianqinum.".shtml";
			// 获取数据
			$content = @file_get_contents($url);
			$content = iconv('GB2312', 'UTF-8//IGNORE', $content);
			// 期号
			preg_match_all('/<font class=\"cfont2\">.*<\/font>/i', $content, $qihao);
			$returnData['data'][$j]['current_number']=strip_tags($qihao[0][0]);
			...
			$notice=str_replace("月", "-", $notice);
			$notice=str_replace("日", "", $notice);
			$returnData['data'][$j]['date']=strip_tags($notice);
			...
			$xiaoliang=str_replace('元', '', $xiaoliang);
			$xiaoliang=str_replace('&nbsp;', '', $xiaoliang);
			$returnData['data'][$j]['sale']=trim($xiaoliang);
			...
			$jiangchiguncun=str_replace('元', '', $jiangchiguncun);
			$jiangchiguncun=str_replace('&nbsp;', '', $jiangchiguncun);
			$returnData['data'][$j]['money_pool']=trim($jiangchiguncun);
			...
			$redballs[]=strip_tags($redball[0][4]);
			$redballs[]=strip_tags($redball[0][5]);
			$returnData['data'][$j]['open_number']=$redballs;
		
			// 蓝球
			preg_match_all('/<li class=\"ball_blue\">.*<\/li>/i', $content, $blueball);
			$blueball=strip_tags($blueball[0][0]);
			if(!is_array($blueball)){
				$returnData['data'][$j]['special_number'] = array($blueball);
			}else{
				$returnData['data'][$j]['special_number'] = $blueball;
			}
			
			
			// 开奖详情
			$returnData['data'][$j]['details'] = array();
			$prizeNum=array("一","二","三","四","五","六");
			$prizecon=array("中6+1","中6+0","中5+1","中5+0/4+1","中4+0/3+1","中2+1/1+1/0+1");
			foreach($prizeNum as $key=>$value){
				preg_match("/" . $value . "等奖<\/td>\s*<td>\s*.*<\/td>\s*<td>\s*.*/",$content,$value);
			...
			$returnData['data'][$j]['details']=$prize;
						
			$j++;
			
		}
		if(!$returnData['data'][0]['current_number']){
...
// 		dump($data);
	...
	public function fucai3dApi(){
		...
		for($i=$qishi;$i<$num;$i++){
			...
			// 奖池滚存
			$returnData['data'][$j]['money_pool'] = 0;
			...
			$balls[]=strip_tags($ball[0][1]);
			$balls[]=strip_tags($ball[0][2]);
			$returnData['data'][$j]['open_number']=$balls;
			
			$returnData['data'][$j]['special_number'] = array();
	...
	public function qilecaiApi(){
		...
		for($i=$qishi;$i<$num;$i++){
			...
			$url = "http://kaijiang.500.com/shtml/qlc/".$dangqianqinum.".shtml";
			...
			preg_match_all('/<li class=\"ball_blue\">.*<\/li>/i', $content, $blueball);
			if(!is_array(strip_tags($blueball[0][0]))){
				$returnData['data'][$j]['special_number']=array(strip_tags($blueball[0][0]));
			}else{
				$returnData['data'][$j]['special_number'] = strip_tags($blueball[0][0]);
			}
			// 中奖条件
			$prizeCon=array("中7+0","中6+1","中6+0","中5+1","中5+0","中4+1");
	...
	public function pailie3Api(){
		...
		for($i=$qishi;$i<$num;$i++){	
			...
			$url = "http://kaijiang.500.com/shtml/pls/".$dangqianqinum.".shtml";
			// 获取数据
			$content = @file_get_contents($url);
			$content = iconv('GB2312', 'UTF-8//IGNORE', $content);			
			// 期号
			preg_match_all('/<font class=\"cfont2\">.*<\/font>/i', $content, $qihao);
			$qihao = strip_tags($qihao[0][0]);
			$returnData['data'][$j]['current_number']=$qihao;
	...
	public function qixingcaiApi(){
		...
		for($i=$qishi;$i<$num;$i++){	
			...
			$url = "http://kaijiang.500.com/shtml/qxc/".$dangqianqinum.".shtml";	
			// 获取数据
			$content = @file_get_contents($url);
			$content = iconv('GB2312', 'UTF-8//IGNORE', $content);
	...
	public function daletouApi(){
		...
		for($i=$qishi;$i<$num;$i++){
			...
			$url = "http://kaijiang.500.com/shtml/dlt/".$dangqianqinum.".shtml";
			...
			$blueballs[]=strip_tags($blueball[0][0]);
			$blueballs[]=strip_tags($blueball[0][1]);
			$returnData['data'][$j]['special_number']=$blueballs;
